...
for (CATEG in c("GS","DNA","Helitron", "LINE", "LTR", "SINE", "TEs")){
  ...
  for (x in c("median_GC3","omega_coevol","omega_mapNH")){
    ...
    if (CATEG=="GS"){print(cor.test(Y,X))}
...
#chi-2
    ...
    chi2_matrix[1,1] <- sum(ifelse(df$pp[df$categ==CATEG & df$proxy=="omega" & df$concat!="Random"]>=0.975,1,0))
    chi2_matrix[1,2] <- sum(ifelse(df$pp[df$categ==CATEG & df$proxy=="omega" & df$concat!="Random"]<0.975,1,0))
    chi2_matrix[2,1] <- sum(ifelse(df$pp[df$categ==CATEG & df$proxy=="omega" & df$concat=="Random"]>=0.975,1,0))
    chi2_matrix[2,2] <- sum(ifelse(df$pp[df$categ==CATEG & df$proxy=="omega" & df$concat=="Random"]<0.975,1,0))
...
NotRandom_omega <- ggplot(df[df$concat!="Random" & df$categ==CATEG & df$proxy=="omega",],
  ...
  geom_segment(data=df[df$concat!="Random" & df$categ==CATEG & df$proxy=="omega",],
               aes(x=min_GC3,xend=max_GC3,y=cc,yend=cc))+
  geom_abline(
    intercept=Intercept,
    slope=Slope,
...
Random_omega <- ggplot(df[df$concat=="Random" & df$categ==CATEG & df$proxy=="omega",],           aes(x=proxy,
  ...
  theme(legend.position="none",
        ...
        axis.title.x=element_blank())+
  ylim(c(MIN_X,MAX_X))+
  scale_colour_manual(values=c("darkgray","#D46A6A",
                                         "#801515"))+
...
legend <- ggplot(df,
  ...
  legend.key = element_blank(), #element_rect(color="black",size=0.15 ),
